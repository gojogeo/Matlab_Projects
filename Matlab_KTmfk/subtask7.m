load("V4_Input_Crane.mat"); 
 ...
 inputparam.ub =ub;
 inputparam.nonlcon =[];
 inputparam.options = optimoptions(@fmincon,'Algorithm','sqp','Display','iter');

 x0 =ones(nTruss,1);
 targetnode = 2*force(1);
 
 nonlin = @(x_opt)ConstraintFunction(x_opt, A, EA, S, nTruss, 2175, 0.080);
 inputparam.nonlcon =nonlin;
